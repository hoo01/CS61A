...
def about(subject):
    ...
    # END PROBLEM 2
    def helper(prgrh):
        prgrh1 = lower(remove_punctuation(prgrh))
        words = split(prgrh1)
        for i in words:
            if i in subject:
                return True
...
def accuracy(typed, source):
    ...
    correct_accuracy = (correct_count/len(typed_words))*100
    return correct_accuracy
...
def feline_fixes(typed, source, limit):
    ...
    """
    # BEGIN PROBLEM 6
    dif_len = abs(len(source) - len(typed))
    def helper(n, dif_cha):
        if dif_cha + dif_len > limit:
            return limit + 1
        if n == min(len(typed), len(source)):
            return dif_cha + dif_len  # 加上长度差异
        t = typed[n]
        s = source[n]
        if t != s:
            dif_cha += 1
        return helper(n + 1, dif_cha)
    # 从位置 0 开始递归
    return helper(0, 0)
    # END PROBLEM 6
############
...
def report_progress(typed, source, user_id, upload):
    ...
    for i in range(len(typed)):
        if typed[i] == source[i]:
            ...
            break
    progress = correct_num/len(source)
...
def fastest_words(match):
    ...
    for i in player_indices:
        ans.append([]) #[],[]
    for j in word_indices: #j =0,1,2,3
        idx = min(player_indices,key = lambda x:time(match,x,j))#idx是得到每个单词用时最短的选手编号
        ans[idx].append(get_word(match,j))
    return ans
    # END PROBLEM 10
